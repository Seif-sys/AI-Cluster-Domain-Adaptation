...
#SBATCH --output=upper_bound_%j.out
#SBATCH --mail-user=<yourEmailId>@tu-braunschweig.de
#SBATCH --mail-type=INVALID_DEPEND,BEGIN,END,FAIL,TIME_LIMIT_50,TIME_LIMIT

# =============================================================================
# run_upper_bound.sh
# Fine-tune on labeled target data — supervised upper bound comparison.
#
# !! This is NOT a UDA method. Target labels ARE used for training. !!
# Its only purpose is to establish the performance ceiling.
#
# Prerequisite: run_source.sh must have completed and produced:
#   ${WORK}/checkpoints/source/best_model.pt
#
# Produces:
#   ${WORK}/checkpoints/upper_bound/best_model.pt
#   ${WORK}/results/upper_bound/results_summary.txt
#   ${WORK}/results/upper_bound/metrics.csv
#
# Usage (interactive, no SLURM):
#   bash scripts/run_upper_bound.sh
#
# Usage (cluster):
#   sbatch scripts/run_upper_bound.sh
# =============================================================================
...
RESULTS_DIR=${WORK}/results/upper_bound
CHECKPOINTS_DIR=${WORK}/checkpoints/upper_bound
...
# ---- Run upper bound fine-tuning --------------------------------------------
echo "Starting target-supervised upper bound fine-tuning..."
echo "  Source checkpoint : $SOURCE_CKPT"
echo "  Results dir       : $RESULTS_DIR"
echo "  Checkpoints dir   : $CHECKPOINTS_DIR"
echo "  Job ID            : $SLURM_JOB_ID"
echo ""
echo "  NOTE: target labels ARE used here. This is the upper bound, not UDA."
...
if [ $EXIT_CODE -eq 0 ]; then
    echo ""
    echo "Upper bound fine-tuning finished successfully."
    ...
    echo "ERROR: train_upper_bound.py exited with code $EXIT_CODE"
    exit $EXIT_CODE
fi
